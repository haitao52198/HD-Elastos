...
allocman_t *bootstrap_new_1level_simple(simple_t *simple, uint32_t pool_size, char *pool) {
    allocman_t *allocman;
    int error;
    /* Initialize inside the current 1 level cspace as defined by simple */
    allocman = bootstrap_new_1level_bootinfo(simple->data, simple_get_cnode_size_bits(simple), pool_size, pool, NULL);
    if (!allocman) {
        return allocman;
    }
...
run_hi(uint32_t size)
{
    ...
    char size_string[10];
    char allocate_size[] = { HI_APP };
    char *argv[] = {sel4test_name, allocate_size};
    argv[0] = endpoint_string;
    argv[1] = size_string;
    snprintf(endpoint_string, 10, "%d", endpoint);
    snprintf(size_string, 10, "%d", size);
...
int main_continued(void)
{
    ...
    /* ================== My Execution ================= */
    /* Hello, sel4test */
    printf("\nHello, sel4test for bootstrap with new 1level!\n");
    printf("==============================================\n\n");

    run_hi(10); // This argument is useless now.
