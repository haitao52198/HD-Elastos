...
{
    int ret = RT_EOK;
    const struct dfs_filesystem_operation **empty = NULL;
...
int dfs_mkfs(const char *fs_name, const char *device_name)
{
    ...
    printf("Can not find the file system which named as %s.\n", fs_name);
    return -1;
}
...
int df(const char *path)
{
    ...
    printf("disk free: %d KB [ %d block, %d bytes per block ]\n",
