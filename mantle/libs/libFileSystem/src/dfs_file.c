...
int dfs_file_close(struct dfs_fd *fd)
{
    ...
    if (fd != NULL && fd->fs->ops->close != NULL)
 ...
 */
int dfs_file_read(struct dfs_fd *fd, void *buf, UInt32 len)
{
    struct dfs_filesystem *fs;
    int result = 0;
 ...
 */
int dfs_file_getdents(struct dfs_fd *fd, struct dirent *dirp, UInt32 nbytes)
{
    struct dfs_filesystem *fs;
 ...
 */
int dfs_file_write(struct dfs_fd *fd, const void *buf, UInt32 len)
{
    struct dfs_filesystem *fs;
 ...
 */
int dfs_file_lseek(struct dfs_fd *fd, Int32 offset)
{
    int result;
