...
#define dfs_log(level, x)        do { if (level & DFS_DEBUG_LEVEL)      \
    {printf("DFS %s, %d:", __FUNCTION__, __LINE__); printf x;   \
    printf ("\n");}}while (0)
#else
...
struct statfs
{
    UInt32 f_bsize;   /* block size */
    UInt32 f_blocks;  /* total data blocks in file system */
    UInt32 f_bfree;   /* free blocks in file system */
};
...
struct dirent
{
    UInt8 d_type;           /* The type of the file */
    UInt8 d_namlen;         /* The length of the not including the terminating null file name */
...
struct dfs_fd
{
    UInt16 magic;           /* file descriptor magic number */
    UInt16 type;            /* Type (regular or socket) */
    ...
    Int32    pos;             /* Current file position */
