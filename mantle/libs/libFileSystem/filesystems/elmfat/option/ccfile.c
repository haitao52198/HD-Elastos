...
int ff_convert_init()
{
    ...
    if (gbk2uni_fd < 0)
        printf("Unable to open GBK to Unicode look up table.\r\n");

    uni2gbk_fd = open(UNI2GBK_FILE, O_RDONLY, 0);
    if (uni2gbk_fd < 0)
        printf("Unable to open Unicode to GBK look up table.\r\n");
	return 0;
}
INIT_APP_EXPORT(ff_convert_init);

UInt16 ff_convert(UInt16 src, UInt32 dir)
{
    if (src < 0x80)
    {
        return src;
