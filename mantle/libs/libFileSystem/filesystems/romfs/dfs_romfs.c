...
struct romfs_dirent *dfs_romfs_lookup(struct romfs_dirent *root_dirent, const char *path, UInt32 *size)
{
	UInt32 index, found;
...
int dfs_romfs_read(struct dfs_fd *file, void *buf, UInt32 count)
{
	UInt32 length;
	struct romfs_dirent *dirent;
...
int dfs_romfs_lseek(struct dfs_fd *file, Int32 offset)
{
	if (offset <= file->size)
	{
		file->pos = offset;
...
int dfs_romfs_getdents(struct dfs_fd *file, struct dirent *dirp, UInt32 count)
{
	UInt32 index;
	const char *name;
	struct dirent *d;
    ...
    if (check_dirent(dirent) != 0)
        return -DFS_STATUS_EIO;
	assert(dirent->type == ROMFS_DIRENT_DIR);
