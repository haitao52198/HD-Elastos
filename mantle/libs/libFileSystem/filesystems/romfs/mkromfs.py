...
class Folder(object):
    ...
    def c_data(self, prefix=''):
        ...
        dtail = '\n};'
        body_fmt = '    {{{type}, "{name}", (UInt8 *){data}, sizeof({data})/sizeof({data}[0])}}'
        # prefix of children
...
const struct romfs_dirent {name} = {{
    ROMFS_DIRENT_DIR, "/", (UInt8 *){rootdirent}, sizeof({rootdirent})/sizeof({rootdirent}[0])
}};
'''
