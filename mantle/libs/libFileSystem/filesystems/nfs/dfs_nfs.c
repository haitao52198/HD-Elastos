...
static void copy_handle(nfs_fh3 *dest, const nfs_fh3 *source)
{
    ...
    dest->data.data_val = malloc(dest->data.data_len);
...
static nfs_fh3 *get_handle(struct nfs_filesystem *nfs, const char *name)
{
    ...
    init = path = malloc(strlen(name)+1);
    if (init == NULL)
        return NULL;

    memcpy(init, name, strlen(name)+1);

    handle = malloc(sizeof(nfs_fh3));
    if (handle == NULL)
    {
        rt_free(init);
...
int nfs_mount(struct dfs_filesystem *fs, unsigned long rwflag, const void *data)
{
    ...
    nfs = (struct nfs_filesystem *)malloc(sizeof(struct nfs_filesystem));
    ...
    {
        printf("host or export path error\n");
        goto __return;
    }
    ...
    {
        printf("create mount client failed\n");
        goto __return;
    }
    ...
    if (nfs->nfs_client == NULL)
    {
        printf("creat nfs client failed\n");
        goto __return;
    }
...
int nfs_read(struct dfs_fd *file, void *buf, UInt32 count)
{
    READ3args args;
    READ3res res;
...
int nfs_write(struct dfs_fd *file, const void *buf, UInt32 count)
{
    WRITE3args args;
    WRITE3res res;
    ssize_t bytes, total=0;
...
int nfs_lseek(struct dfs_fd *file, Int32 offset)
{
    nfs_file *fd;
...
int nfs_open(struct dfs_fd *file)
{
    ...
    if (file->flags & DFS_O_DIRECTORY)
    {
        ...
        if (dir == NULL) return -DFS_STATUS_ENOENT;
        file->data = dir;
    }
    else
    {
...
int nfs_getdents(struct dfs_fd *file, struct dirent *dirp, UInt32 count)
{
    nfs_dir *dir;
    UInt32 index;
    struct dirent *d;
