...
ALIGN(RT_ALIGN_SIZE)
static UInt8 vcom_thread_stack[512];
...
static UInt8 vcom_tx_thread_mq_pool[(VCOM_MQ_MSG_SZ+sizeof(void*))*VCOM_MQ_MAX_MSG];
 ...
 * @return RT_EOK.
 */
static Int32 _ep_cmd_handler(ufunction_t func, UInt32 size)
{
    assert(func != NULL);
 ...
 */
static Int32 _cdc_get_line_coding(udevice_t device, ureq_t setup)
{
...
static Int32 _cdc_set_line_coding_callback(udevice_t device, UInt32 size)
{
 ...
 * @return RT_EOK on successful.
 */
static Int32 _cdc_set_line_coding(udevice_t device, ureq_t setup)
{
    assert(device != NULL);
    assert(setup != NULL);
...
ufunction_t rt_usbd_function_cdc_create(udevice_t device)
{
    ...
    rt_usbd_altsetting_config_descriptor(comm_setting, &_comm_desc,
                                         (Int32)&((ucdc_comm_desc_t)0)->intf_desc);
...
*/
static Int32 _vcom_configure(struct rt_serial_device *serial,
                                struct serial_configure *cfg)
{
    return RT_EOK;
}

static Int32 _vcom_control(struct rt_serial_device *serial,
                              int cmd, void *arg)
{
    switch (cmd)
    {
