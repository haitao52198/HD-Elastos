 ...
 * @return the error code, RT_EOK on successfully.
 */
Int32 rt_usbh_hub_get_port_status(uhub_t hub, UInt16 port,
    UInt8* buffer)
{
    struct ureqest setup;
    int timeout = 100;
 ...
 * @return the error code, RT_EOK on successfully.
 */
Int32 rt_usbh_hub_clear_port_feature(uhub_t hub, UInt16 port,
    UInt16 feature)
{
    struct ureqest setup;
    int timeout = 100;
    ...
    if(rt_usb_hcd_control_xfer(hub->hcd, hub->self, &setup, NULL, 0,
 ...
 * @return the error code, RT_EOK on successfully.
 */
Int32 rt_usbh_hub_set_port_feature(uhub_t hub, UInt16 port,
    UInt16 feature)
{
    struct ureqest setup;
    int timeout = 100;
 ...
 * @return the error code, RT_EOK on successfully.
 */
Int32 rt_usbh_hub_reset_port(uhub_t hub, UInt16 port)
{
    Int32 ret;
    UInt32 pstatus;
    ...
    while(1)
    {
        ret = rt_usbh_hub_get_port_status(hub, port, (UInt8*)&pstatus);
 ...
 * @return the error code, RT_EOK on successfully.
 */
Int32 rt_usbh_hub_port_debounce(uhub_t hub, UInt16 port)
{
    Int32 ret;
    int i = 0, times = 20;
    UInt32 pstatus;
 ...
 * @return the error code, RT_EOK on successfully.
 */
static Int32 rt_usbh_hub_port_change(uhub_t hub)
{
    int i;
    rt_bool_t reconnect;
    ...
    for (i = 0; i < hub->num_ports; i++)
    {
        ...
        /* get hub port status */
        ret = rt_usbh_hub_get_port_status(hub, i + 1, (UInt8*)&pstatus);
...
static void rt_usbh_hub_irq(void* context)
{
    ...
    /* parameter check */
     assert(pipe->intf->device->hcd != NULL);
 ...
 * @return the error code, RT_EOK on successfully.
 */
static Int32 rt_usbh_hub_enable(void *arg)
{
    int i = 0;
    Int32 ret = RT_EOK;
    uep_desc_t ep_desc;
    ...
    ret = rt_usbh_hub_get_descriptor(device, (UInt8*)&hub->hub_desc, 8);
    if(ret != RT_EOK)
    {
    ...
    ret = rt_usbh_hub_get_descriptor(device, (UInt8*)&hub->hub_desc,
        hub->hub_desc.length);
    if(ret != RT_EOK)
    {
        printf("get hub descriptor again failed\n");
        return -RT_ERROR;
    }
 ...
 * @return the error code, RT_EOK on successfully.
 */
static Int32 rt_usbh_hub_disable(void* arg)
{
    int i;
    uhub_t hub;
    struct uinstance* device;
    ...
    if(hub != NULL) rt_free(hub);
    if(intf != NULL) rt_free(intf);
...
void rt_usbh_hub_init(void)
{
    ...
    thread = rt_thread_create("usbh", rt_usbh_hub_thread_entry, NULL,
