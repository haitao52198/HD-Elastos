 ...
 *
 * @return the allocate instance on successful, or NULL on failure.
 */
 ...
 */
Int32 rt_usbh_attatch_instance(uinst_t device)
{
    int i = 0;
    Int32 ret = RT_EOK;
    ...
    if(ret != RT_EOK)
    {
        printf("get device descriptor head failed\n");
        return ret;
    }
    ...
    if(ret != RT_EOK)
    {
        printf("get full device descriptor failed\n");
        return ret;
    }
    ...
    if(ret != RT_EOK)
    {
        printf("get configuration descriptor head failed\n");
        return ret;
    }

    /* alloc memory for configuration descriptor */
    device->cfg_desc = (ucfg_desc_t)malloc(cfg_desc.wTotalLength);
    ...
    if(ret != RT_EOK)
    {
        printf("get full configuration descriptor failed\n");
        return ret;
    }
    ...
    {        
        ...
        if(ret != RT_EOK)
        {
            printf("rt_usb_get_interface_descriptor error\n");
            return -RT_ERROR;
        }
 ...
 * @return the error code, RT_EOK on successfully.
 */
Int32 rt_usbh_detach_instance(uinst_t device)
{
    int i = 0;

    if(device == NULL) 
    {
        printf("no usb instance to detach\n");
        return -RT_ERROR;
    }
    ...
    for(i=0; i<device->cfg_desc->bNumInterfaces; i++)
    {
        if(device->intf[i] == NULL) continue;
        if(device->intf[i]->drv == NULL) continue;

        assert(device->intf[i]->device == device);
 ...
 * @return the error code, RT_EOK on successfully.
 */
Int32 rt_usbh_get_descriptor(uinst_t device, UInt8 type, void* buffer, 
    int nbytes)
{
 ...
 * @return the error code, RT_EOK on successfully.
 */
Int32 rt_usbh_set_address(uinst_t device)
{
    struct ureqest setup;
    int timeout = 100;
    ...
    if(rt_usb_hcd_control_xfer(device->hcd, device, &setup, NULL, 0, 
 ...
 * @return the error code, RT_EOK on successfully.
 */
Int32 rt_usbh_set_configure(uinst_t device, int config)
{
    struct ureqest setup;
    int timeout = 100;
 ...
 * @return the error code, RT_EOK on successfully.
 */
Int32 rt_usbh_set_interface(uinst_t device, int intf)
{
    struct ureqest setup;
    int timeout = 100;
 ...
 * @return the error code, RT_EOK on successfully.
 */
Int32 rt_usbh_clear_feature(uinst_t device, int endpoint, int feature)
{
    struct ureqest setup;
    int timeout = 100;
    ...
    uintf_desc_t* intf_desc)
{
    ...
    /* check parameter */
    assert(cfg_desc != NULL);

    ptr = (UInt32)cfg_desc + cfg_desc->bLength;
    while(ptr < (UInt32)cfg_desc + cfg_desc->wTotalLength)
    {
        if(depth++ > 0x20) 
        {
            *intf_desc = NULL;        
    ...
    printf("rt_usb_get_interface_descriptor %d failed\n", num);
    return -RT_EIO;
}
    ...
    uep_desc_t* ep_desc)
{
    ...
    ptr = (UInt32)intf_desc + intf_desc->bLength;
    ...
    printf("rt_usb_get_endpoint_descriptor %d failed\n", num);
    return -RT_EIO;
}
