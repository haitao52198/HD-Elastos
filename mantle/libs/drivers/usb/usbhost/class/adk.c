...
static const char* _adk_manufacturer = NULL;
static const char* _adk_model = NULL;
static const char* _adk_description = NULL;
static const char* _adk_version = NULL;
static const char* _adk_uri = NULL;
static const char* _adk_serial = NULL;
...
*/
static Int32 rt_usbh_adk_get_protocol(struct uintf* intf, UInt16 *protocol)
{
 ...
 * @return the error code, RT_EOK on successfully.
*/
static Int32 rt_usbh_adk_send_string(struct uintf* intf, UInt16 index,
    const char* str)
{
    struct ureqest setup;
    uinst_t device;
 ...
 * @return the error code, RT_EOK on successfully.
*/
static Int32 rt_usbh_adk_start(struct uintf* intf)
{
    struct ureqest setup;
    uinst_t device;
    int timeout = 100;
 ...
 * @return the error code, RT_EOK on successfully.
*/
static UInt32 rt_usbh_adk_read(rt_device_t device, Int32 pos, void* buffer,
    UInt32 size)
{
    uadk_t adk;
    UInt32 length;
 ...
 * @return the error code, RT_EOK on successfully.
*/
static UInt32 rt_usbh_adk_write (rt_device_t device, Int32 pos, const void* buffer,
    UInt32 size)
{
    uadk_t adk;
    UInt32 length;
 ...
 * @return the error code, RT_EOK on successfully.
 */
static Int32 rt_usbh_adk_enable(void* arg)
{
    int i = 0;
    uadk_t adk;
    ...
    if(intf == NULL)
    {
        printf("the interface is not available\n");
        return -RT_EIO;
    }
    ...
    else
    {
        ...
        {
            printf("rt_usbh_adk_get_protocol failed\n");
            return ret;
        }

        if(protocol != 1)
        {
            printf("read protocol failed\n");
            return -RT_ERROR;
        }
        ...
        {
            printf("rt_usbh_adk_start failed\n");
            return ret;
        }
    ...
    adk = malloc(sizeof(struct uadkinst));
    assert(adk != NULL);
    ...
    for(i=0; i<intf->intf_desc->bNumEndpoints; i++)
    {
        ...
        if(ep_desc == NULL)
        {
            printf("rt_usb_get_endpoint_descriptor error\n");
            return -RT_ERROR;
        }
    ...
    if(adk->pipe_in == NULL || adk->pipe_out == NULL)
    {
        printf("pipe error, unsupported device\n");
        return -RT_ERROR;
    }
    ...
    adk->device.init = NULL;
    adk->device.open = NULL;
    adk->device.close = NULL;
    adk->device.read = rt_usbh_adk_read;
    adk->device.write = rt_usbh_adk_write;
    adk->device.control = NULL;
 ...
 * @return the error code, RT_EOK on successfully.
 */
static Int32 rt_usbh_adk_disable(void* arg)
{
    uadk_t adk;
    struct uintf* intf = (struct uintf*)arg;
