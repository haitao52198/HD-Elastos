...
static Int32 mmcsd_num_wr_blocks(struct rt_mmcsd_card *card)
{
    Int32 err;
    UInt32 blocks;
...
static Int32 rt_mmcsd_req_blk(struct rt_mmcsd_card *card,
                                 UInt32           sector,
                                 void                 *buf,
                                 UInt32             blks,
                                 UInt8            dir)
...
static Int32 rt_mmcsd_init(rt_device_t dev)
{
    return RT_EOK;
}

static Int32 rt_mmcsd_open(rt_device_t dev, UInt16 oflag)
{
    return RT_EOK;
}

static Int32 rt_mmcsd_close(rt_device_t dev)
{
    return RT_EOK;
}

static Int32 rt_mmcsd_control(rt_device_t dev, UInt8 cmd, void *args)
{
...
static UInt32 rt_mmcsd_read(rt_device_t dev,
                               Int32    pos,
                               void       *buffer,
                               UInt32   size)
{
...
static UInt32 rt_mmcsd_write(rt_device_t dev,
                                Int32    pos,
                                const void *buffer,
                                UInt32   size)
{
...
static Int32 mmcsd_set_blksize(struct rt_mmcsd_card *card)
{
    struct rt_mmcsd_cmd cmd;
    int err;
    ...
    if (err)
    {
        printf("MMCSD: unable to set block size to %d: %d\n", cmd.arg, err);
...
Int32 rt_mmcsd_blk_probe(struct rt_mmcsd_card *card)
{
    ...
    sector = (UInt8 *)malloc(SECTOR_SIZE);
    if (sector == NULL)
    {
        printf("allocate partition sector buffer failed\n");
    ...
    if (status == RT_EOK)
    {
        ...
        {
            blk_dev = malloc(sizeof(struct mmcsd_blk_device));
            if (!blk_dev)
            {
                printf("mmcsd:malloc mem failde\n");
                break;
            }
            memset((void *)blk_dev, 0, sizeof(struct mmcsd_blk_device));
    ...
    {
        printf("read mmcsd first sector failed\n");
        err = -RT_ERROR;
    }
