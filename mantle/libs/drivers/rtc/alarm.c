...
rt_inline UInt32 alarm_mkdaysec(struct tm *time)
{
    UInt32 sec;
...
static Int32 alarm_set(struct rt_alarm *alarm)
{
    rt_device_t device;
...
static void alarm_wakeup(struct rt_alarm *alarm, struct tm *now)
{
    ...
    if (alarm->flag & RT_ALARM_STATE_START)
    {
        ...
        if ((wakeup == RT_TRUE) && (alarm->callback != NULL))
        {
            timestamp = time(NULL);
...
static void alarm_update(UInt32 event)
{
    struct rt_alarm *alm_prev = NULL, *alm_next = NULL;
    struct rt_alarm *alarm;
    Int32 sec_now, sec_alarm, sec_tmp;
    Int32 sec_next = 24 * 3600, sec_prev = 0;
    time_t timestamp;
    struct tm now;
    rt_list_t *next;
...
static UInt32 days_of_year_month(int tm_year, int tm_mon)
{
    UInt32 ret, year;
...
static Int32 alarm_setup(rt_alarm_t alarm, struct tm *wktime)
{
    Int32 ret = RT_ERROR;
    time_t timestamp;
    struct tm *setup, now;
 ...
 * \return none
 */
void rt_alarm_update(rt_device_t dev, UInt32 event)
{
 ...
 * \param arg argument
 */
Int32 rt_alarm_control(rt_alarm_t alarm, UInt8 cmd, void *arg)
{
    Int32 ret = RT_ERROR;
 ...
 * \return RT_EOK
 */
Int32 rt_alarm_start(rt_alarm_t alarm)
{
    Int32 sec_now, sec_old, sec_new;
    Int32 ret = RT_ERROR;
    time_t timestamp;
    struct tm now;
 ...
 * \return RT_EOK
 */
Int32 rt_alarm_stop(rt_alarm_t alarm)
{
    Int32 ret = RT_ERROR;
 ...
 * \return RT_EOK
 */
Int32 rt_alarm_delete(rt_alarm_t alarm)
{
    Int32 ret = RT_ERROR;
...
rt_alarm_t rt_alarm_create(rt_alarm_callback_t callback, struct rt_alarm_setup *setup)
{
    ...
    if (setup == NULL)
        return (NULL);
    alarm = malloc(sizeof(struct rt_alarm));
    if (alarm == NULL)
        return (NULL);
