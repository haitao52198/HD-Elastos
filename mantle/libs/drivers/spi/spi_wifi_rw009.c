...
#ifdef WIFI_DEBUG_ON
#define WIFI_DEBUG         printf("[RW009] ");printf
//#define SPI_DEBUG         printf("[SPI] ");printf
#define SPI_DEBUG(...)
#else
#define WIFI_DEBUG(...)
#define SPI_DEBUG(...)
...
struct rw009_wifi
{
    ...
    ALIGN(4)
    UInt8 spi_tx_mempool[(sizeof(struct spi_data_packet) + 4) * SPI_TX_POOL_SIZE];
    ALIGN(4)
    UInt8 spi_rx_mempool[(sizeof(struct spi_data_packet) + 4) * SPI_RX_POOL_SIZE];
...
{
    struct rw009_resp *resp_return = NULL;

    switch (resp->cmd)
    {
    case RW009_CMD_INIT:
        WIFI_DEBUG("resp_handler RW009_CMD_INIT\n");
        resp_return = (struct rw009_resp *)malloc(member_offset(struct rw009_resp, resp) + sizeof(rw009_resp_init)); //TODO:
        if(resp_return == NULL) break;
        ...
        memcpy(wifi_device->dev_addr, resp->resp.init.mac, 6);
        break;
    ...
    case RW009_CMD_EASY_JOIN:
        ...
        resp_return = (struct rw009_resp *)malloc(member_offset(struct rw009_resp, resp) + sizeof(rw009_resp_join)); //TODO:
        if(resp_return == NULL) break;
    ...
    {
        rt_mb_send(&wifi_device->rw009_cmd_mb, (UInt32)resp_return);
        return;
    }
    else
    {
...
static Int32 rw009_cmd(struct rw009_wifi *wifi_device, uint32_t cmd, void *args)
{
    Int32 result = RT_EOK;
    Int32 timeout = RW009_CMD_TIMEOUT;

    struct spi_data_packet *data_packet;
    struct rw009_cmd *wifi_cmd = NULL;
    struct rw009_resp *resp = NULL;
    ...
    rt_mb_send(&wifi_device->spi_tx_mb, (UInt32)data_packet);
...
static Int32 spi_wifi_transfer(struct rw009_wifi *dev)
{
    struct pbuf *p = NULL;
    ...
    Int32 result;
    const struct spi_data_packet *data_packet = NULL;
    ...
    result = rt_mb_recv(&wifi_device->spi_tx_mb,
                        (UInt32 *)&data_packet,
                        0);
    if ((result == RT_EOK) && (data_packet != NULL) && (data_packet->data_len > 0))
    {
        ...
        if ((resp.S2M_len) && (resp.S2M_len <= MAX_SPI_PACKET_SIZE))
        {
            ...
            if (data_packet->data_type == data_type_eth_data)
            {
                ...
                if (wifi_device->active)
                {
                    ...
                    pbuf_take(p, (UInt8 *)data_packet->buffer, data_packet->data_len);

                    rt_mb_send(&wifi_device->eth_rx_mb, (UInt32)p);
...
static Int32 rw009_wifi_init(rt_device_t dev)
{
    return RT_EOK;
}

static Int32 rw009_wifi_open(rt_device_t dev, UInt16 oflag)
{
    return RT_EOK;
}

static Int32 rw009_wifi_close(rt_device_t dev)
{
    return RT_EOK;
}

static UInt32 rw009_wifi_read(rt_device_t dev, Int32 pos, void *buffer, UInt32 size)
{
    rt_set_errno(-RT_ENOSYS);
    return 0;
}

static UInt32 rw009_wifi_write(rt_device_t dev, Int32 pos, const void *buffer, UInt32 size)
{
    rt_set_errno(-RT_ENOSYS);
    return 0;
}

static Int32 rw009_wifi_control(rt_device_t dev, UInt8 cmd, void *args)
{
    struct rw009_wifi *wifi_device = (struct rw009_wifi *)dev;
    Int32 result = RT_EOK;
...
/* transmit packet. */
Int32 rw009_wifi_tx(rt_device_t dev, struct pbuf *p)
{
    Int32 result = RT_EOK;
    struct spi_data_packet *data_packet;
    struct rw009_wifi *wifi_device = (struct rw009_wifi *)dev;
...
struct pbuf *rw009_wifi_rx(rt_device_t dev)
{
    ...
    if (rt_mb_recv(&wifi_device->eth_rx_mb, (UInt32 *)&p, 0) != RT_EOK)
    {
        return NULL;
    }
...
Int32 rt_hw_wifi_init(const char *spi_device_name, wifi_mode_t mode)
{
    /* align and struct size check. */
    assert( (SPI_MAX_DATA_LEN & 0x03) == 0);
    assert( sizeof(struct rw009_resp) <= SPI_MAX_DATA_LEN);
    ...
    if (rw009_wifi_device.rt_spi_device == NULL)
    {
    ...
    rw009_wifi_device.parent.parent.user_data  = NULL;
...
/********************************* RW009 tools **************************************/
Int32 rw009_join(const char * SSID, const char * passwd)
{
    Int32 result;
...
Int32 rw009_softap(const char * SSID, const char * passwd,uint32_t security,uint32_t channel)
{
    Int32 result;
    rt_device_t wifi_device;
...
static Int32 rw009_scan(void)
{
    ...
    printf("CMD RW009_CMD_SCAN result:%d\n", result);
