...
struct enc28j60_tx_list_typedef
{
    ...
    UInt32 addr; /* pkt addr in buffer */
    UInt32 len;  /* pkt len */
...
static void spi_write(struct rt_spi_device * spi_device, UInt8 address, UInt8 data);

static void enc28j60_clkout(struct rt_spi_device * spi_device, UInt8 clk);
...
static uint16_t enc28j60_phy_read(struct rt_spi_device * spi_device, UInt8 address);
static void enc28j60_phy_write(struct rt_spi_device * spi_device, UInt8 address, uint16_t data);
...
static Int32 enc28j60_init(rt_device_t dev)
{
...
static Int32 enc28j60_control(rt_device_t dev, UInt8 cmd, void *args)
{
    struct net_device * enc28j60 = (struct net_device *)dev;
    switch(cmd)
    {
...
static Int32 enc28j60_open(rt_device_t dev, uint16_t oflag)
{
    return RT_EOK;
}

/* Close the interface */
static Int32 enc28j60_close(rt_device_t dev)
{
    return RT_EOK;
}

/* Read */
static UInt32 enc28j60_read(rt_device_t dev, Int32 pos, void* buffer, UInt32 size)
{
...
/* Write */
static UInt32 enc28j60_write(rt_device_t dev, Int32 pos, const void* buffer, UInt32 size)
{
    rt_set_errno(-RT_ENOSYS);
    return 0;
}
...
/* Transmit packet. */
static Int32 enc28j60_tx( rt_device_t dev, struct pbuf* p)
{
    struct net_device * enc28j60 = (struct net_device *)dev;
    struct rt_spi_device * spi_device = enc28j60->spi_device;
    struct pbuf* q;
...
//            spi_write_op(spi_device, ENC28J60_BIT_FIELD_CLR, EIR, EIR_RXERIF);
    ...
    if(pk_counter)
    {
        ...
        rxstat |= ((UInt16)spi_read_op(spi_device, ENC28J60_READ_BUF_MEM, 0))<<8;
...
Int32 enc28j60_attach(const char * spi_device_name)
{
    struct rt_spi_device * spi_device;
...
static void enc28j60(void)
{
    ...
    printf("MAC  : MACON1 MACON3 MACON4\n");
