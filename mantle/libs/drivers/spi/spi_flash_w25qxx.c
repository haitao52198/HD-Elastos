...
uint32_t w25qxx_page_write(uint32_t page_addr, const uint8_t* buffer)
{
    ...
    assert((page_addr&0xFF) == 0); /* page addr must align to 256byte. */
...
static Int32 w25qxx_flash_init(rt_device_t dev)
{
    return RT_EOK;
}

static Int32 w25qxx_flash_open(rt_device_t dev, UInt16 oflag)
{
...
static Int32 w25qxx_flash_close(rt_device_t dev)
{
    return RT_EOK;
}

static Int32 w25qxx_flash_control(rt_device_t dev, UInt8 cmd, void *args)
{
    assert(dev != NULL);
...
static UInt32 w25qxx_flash_read(rt_device_t dev,
                                   Int32 pos,
                                   void* buffer,
                                   UInt32 size)
{
...
static UInt32 w25qxx_flash_write(rt_device_t dev,
                                    Int32 pos,
                                    const void* buffer,
                                    UInt32 size)
{
...
Int32 w25qxx_init(const char * flash_device_name, const char * spi_device_name)
{
    ...
    {
        printf("init sd lock mutex failed\n");
        return -RT_ENOSYS;
    }
    ...
    /* no private */
    spi_flash_device.flash_device.user_data = NULL;
