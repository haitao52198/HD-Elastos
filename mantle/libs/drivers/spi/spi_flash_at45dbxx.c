...
static Int32 AT45DB_flash_init(rt_device_t dev)
{
    return RT_EOK;
}

static Int32 AT45DB_flash_open(rt_device_t dev, UInt16 oflag)
{

    return RT_EOK;
}

static Int32 AT45DB_flash_close(rt_device_t dev)
{
    return RT_EOK;
}

static Int32 AT45DB_flash_control(rt_device_t dev, UInt8 cmd, void *args)
{
    assert(dev != NULL);
...
static UInt32 AT45DB_flash_read_page_256(rt_device_t dev, Int32 pos, void* buffer, UInt32 size)
{
    uint32_t index, nr;
...
static UInt32 AT45DB_flash_read_page_512(rt_device_t dev, Int32 pos, void* buffer, UInt32 size)
{
    uint32_t index, nr;
    uint8_t * read_buffer = buffer;
...
static UInt32 AT45DB_flash_read_page_1024(rt_device_t dev, Int32 pos, void* buffer, UInt32 size)
{
    uint32_t index, nr;
    uint8_t * read_buffer = buffer;
...
static UInt32 AT45DB_flash_write_page_256(rt_device_t dev, Int32 pos, const void* buffer, UInt32 size)
{
    UInt32 index, nr;
...
static UInt32 AT45DB_flash_write_page_512(rt_device_t dev, Int32 pos, const void* buffer, UInt32 size)
{
    UInt32 index, nr;
    const uint8_t * write_buffer = buffer;
...
static UInt32 AT45DB_flash_write_page_1024(rt_device_t dev, Int32 pos, const void* buffer, UInt32 size)
{
    UInt32 index, nr;
    const uint8_t * write_buffer = buffer;
...
Int32 at45dbxx_init(const char * flash_device_name, const char * spi_device_name)
{
    struct rt_spi_device * rt_spi_device;
    struct JEDEC_ID * JEDEC_ID;

    rt_spi_device = (struct rt_spi_device *)rt_device_find(spi_device_name);
    if(rt_spi_device == NULL)
    {
    ...
    /* no private */
    spi_flash_at45dbxx.flash_device.user_data = NULL;
