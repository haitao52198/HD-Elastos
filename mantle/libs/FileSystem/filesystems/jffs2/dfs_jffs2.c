...
static int jffs2_result_to_dfs(int result)
{
	...
	case EMFILE: /** No more file handles available(too many open files)  */
		printf("dfs_jffs2.c error: no more file handles available\r\n");
		status = -1;
		break;//fixme
				    ...
				    const void* data)
{
	...
	{
		if (device_partition[index].dev == NULL)
			break;
	}
	if (index == DEVICE_PART_MAX) 
		return -DFS_STATUS_ENOSPC;

	mte = malloc(sizeof(struct cyg_mtab_entry));
	if (mte == NULL)
...
static int _find_fs(struct cyg_mtab_entry ** mte, rt_device_t dev_id)
{
	...
	printf("error, could not found the fs!");
	return -1;
}
...
static int dfs_jffs2_read(struct dfs_fd* file, void* buf, UInt32 len)
{
	cyg_file * jffs2_file;
...
/* fixme warning: the offset is Int32, so maybe the size of a file is must <= 2G*/
...
int dfs_jffs2_init(void)
{
	...
	{
		printf("init jffs2 lock mutex failed\n");
	}
	printf("init jffs2 lock mutex okay\n");
	return 0;
}
