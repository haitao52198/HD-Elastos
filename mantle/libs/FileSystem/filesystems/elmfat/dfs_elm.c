...
int dfs_elm_unmount(struct dfs_filesystem *fs)
{
    ...
    result = f_mount((BYTE)index, NULL);
    if (result != FR_OK)
        return elm_result_to_dfs(result);
...
int dfs_elm_read(struct dfs_fd *file, void *buf, UInt32 len)
{
    FIL *fd;
    FRESULT result;
    UINT byte_read;
...
int dfs_elm_write(struct dfs_fd *file, const void *buf, UInt32 len)
{
    FIL *fd;
    FRESULT result;
    UINT byte_write;
...
int dfs_elm_lseek(struct dfs_fd *file, Int32 offset)
{
...
int dfs_elm_getdents(struct dfs_fd *file, struct dirent *dirp, UInt32 count)
{
    DIR *dir;
    FILINFO fno;
    FRESULT result;
...
DRESULT disk_ioctl(BYTE drv, BYTE ctrl, void *buff)
{
    ...
    {
        rt_device_control(device, RT_DEVICE_CTRL_BLK_SYNC, NULL);
    }
