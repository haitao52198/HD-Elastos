...
Int32 ping(char* target, UInt32 time, UInt32 size)
{
    int s;
    ...
    while (1)
    {
        ...
        {
            printf("ping: send %d.%d.%d.%d\n", addr->addr0, addr->addr1, addr->addr2, addr->addr3);
            ping_recv(s);
        }
        else
        {
            printf("ping: send %d.%d.%d.%d - error\n", addr->addr0, addr->addr1, addr->addr2, addr->addr3);
        }
